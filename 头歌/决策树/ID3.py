...
class DecisionTree(object):
    ...
    def predict(self, feature):
        ...
        '''
        #************* Begin ************#
        #如果tree是叶子结点，返回叶子结点的值
        def predictOne(tree, feature):  #利用递归不断走到叶子结点确定label的过程
            if not isinstance(tree, dict):  #tree不是字典，说明是叶子结点，直接返回叶子结点的值
                return tree
            for k, v in tree.items(): #k是特征索引，表示当前用哪个属性作为划分，v是字典，表示决策树
                return predictOne(v[feature[k]], feature) #v[feature[k]]表示根据特征值找到下一个子树

        result = []
        for f in feature:  #对于每一个测试集样本，预测其label
            result.append(predictOne(self.tree, f))
