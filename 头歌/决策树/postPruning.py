...
class DecisionTree(object):
    ...
    def predict(self, feature):
        ...
        def classify(tree, feature):
            if not isinstance(tree, dict):
                return tree
            t_index, t_value = list(tree.items())[0] #取出根节点的特征索引和特征值
            f_value = feature[t_index]  #取出当前样本的特征值
            if isinstance(t_value, dict):   #如果根节点的值是字典，说明还有子树
                classLabel = classify(tree[t_index][f_value], feature) #递归调用
                return classLabel
            else:
                return t_value  #如果根节点的值不是字典，说明是叶子结点，直接返回叶子结点的值
