...
def calcInfoGainRatio(feature, label, index):
    ...
    #********* Begin *********#
    V=set(feature[:,index])  #该属性的不同的可取值集合
    D=len(label)   #所有样本个数
    g=0
    for v in V:   #遍历每一个该属性的可取值
        #np.sum(feature[:,index]==v)可以统计出该属性取值为v的样本总共有多少个，即D_v的值
        D_v_D=np.sum(feature[:,index]==v)/D  #D_v/D
        g-=D_v_D*np.log2(D_v_D)
