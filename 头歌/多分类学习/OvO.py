...
class tiny_logistic_regression(object):
    ...
    def predict_proba(self, X):
        #X.shape=(1,4)
        #len(X)=1
        X_b = np.hstack([np.ones((len(X), 1)), X])  # 添加一列全为1的列, X_b = [1, X]
...
class OvO(object):
    ...
    def fit(self, train_datas, train_labels):
        ...
        for i in range(len(unique_labels)):
            for j in range(i+1,len(unique_labels)):
                #两两类别(i类和j类)训练一个二分类器
                data=train_datas[(train_labels==unique_labels[i]) | (train_labels==unique_labels[j])]
                label=train_labels[(train_labels==unique_labels[i]) | (train_labels==unique_labels[j])]
    ...
    def predict(self,test_datas):
        ...
        def _predict(models, test_datas):
            '''
            OvO的预测阶段
            ...
            #********* Begin *********#
            test_datas=np.reshape(test_datas,(1,-1))  #要先转为1行的行向量
            vote={}
            for model in models:
                #type(model.predict(test_datas))=np.ndarray
                #例如：model.predict(test_datas)=[1]，即预测出的类别为1
                #故pred=model.predict(test_datas)[0]=1，把其中的1这个数字取出来
                pred=model.predict(test_datas)[0]  #pred是一个数字,表示预测的类别，如1
                if pred not in vote:
                    vote[pred]=1
                else:
                    vote[pred]+=1
